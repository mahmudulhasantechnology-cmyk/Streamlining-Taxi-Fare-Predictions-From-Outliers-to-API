...
if calculate_btn:
    # A tiny fake spinner delay simulates an active "computing engine" which feels premium to users
    with st.spinner("Filtering anomalies & computing fare..."):
        time.sleep(0.4) 
        
        input_data = np.array([[trip_distance]])
        prediction = model.predict(input_data)
        fare_value = float(np.ravel(prediction))
